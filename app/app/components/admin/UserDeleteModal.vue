    ...
    <div class="fixed inset-0 z-[70] flex items-center justify-center p-4 sm:p-6" role="dialog" aria-modal="true">
      ...
      <div class="relative w-full max-w-md overflow-hidden rounded-2xl bg-surface-container-lowest shadow-2xl">
        <div class="p-6">
          ...
          <h2 class="mb-2 text-title-lg font-bold text-on-surface">Hapus User?</h2>
          <p class="text-body-sm text-on-surface-variant">
            User <span class="font-bold text-on-surface">{{ displayName }}</span> akan dihapus dari daftar aktif.
          </p>
        ...
        <div class="flex items-center gap-2 border-t border-outline-variant bg-surface-container-low px-6 py-4">
          ...
          <p class="text-[11px] leading-tight text-on-surface-variant">Aksi hapus menggunakan soft delete dan token aktif user akan dicabut oleh API.</p>
        </div>
      </div>
    </div>
  </Teleport>
...
const displayName = computed(() => props.user?.nama || props.user?.username || 'User')
...
const deleteUser = async () => {
  ...
  try {
    await $api.delete(`/users/${props.user.user_id}`)
    successMessage.value = 'User berhasil dihapus.'
    setTimeout(() => emit('deleted'), 250)
  } catch (error) {
    errorMessage.value = error?.response?.data?.message || error?.message || 'Terjadi kesalahan saat menghapus user.'
  } finally {
